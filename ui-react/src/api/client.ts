...
class ApiClient {
    ...
    async getHistory(sourceId: string, limit = 100): Promise<any[]> {
        const res = await fetch(`${BASE_URL}/data/${sourceId}/history?limit=${limit}`);
        if (!res.ok) return [];
        return res.json();
    }

    async refreshSource(sourceId: string): Promise<void> {
        const res = await fetch(`${BASE_URL}/refresh/${sourceId}`, { method: 'POST' });
    ...
    async getAuthorizeUrl(sourceId: string, redirectUri: string): Promise<{ authorize_url: string }> {
        const params = new URLSearchParams({ redirect_uri: redirectUri });
        const res = await fetch(`${BASE_URL}/oauth/authorize/${sourceId}?${params}`);
        if (!res.ok) {
            const err = await res.json().catch(() => ({}));
    ...
    async getIntegrationTemplates(integrationId: string): Promise<ViewComponent[]> {
        const res = await fetch(`${BASE_URL}/integrations/${integrationId}/templates`);
        if (!res.ok) throw new Error(`Failed to fetch templates for ${integrationId}`);
        return res.json();
    }
    ...
    async getIntegrationFile(filename: string): Promise<{ filename: string; content: string }> {
        const res = await fetch(`${BASE_URL}/integrations/files/${filename}`);
        if (!res.ok) throw new Error(`Failed to fetch integration ${filename}`);
        return res.json();
    }

    async createIntegrationFile(filename: string, content: string = ""): Promise<void> {
        const res = await fetch(`${BASE_URL}/integrations/files?filename=${encodeURIComponent(filename)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    async getIntegrationSources(filename: string): Promise<any[]> {
        const res = await fetch(`${BASE_URL}/integrations/files/${filename}/sources`);
    ...
    async getSourceFile(filename: string): Promise<{ filename: string; content: string }> {
        const res = await fetch(`${BASE_URL}/sources/files/${filename}`);
        if (!res.ok) throw new Error(`Failed to fetch source ${filename}`);
        return res.json();
    }

    async createSourceFile(config: { name: string; integration_id?: string; vars?: Record<string, any> }): Promise<any> {
        // Auto-generate ID from name
        const id = config.name.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
    ...
    async reloadConfig(): Promise<{ message: string; affected_sources: string[] }> {
